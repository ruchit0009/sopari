                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-3">
                        @if (count($roles) > 0)
                            ...
                            @if($role->id == 2)
                            <input type="radio" value="{{ $role->id }}" checked="checked" name="assignees_roles" id="role-{{ $role->id }}" {{ is_array(old('assignees_roles')) && in_array($role->id, old('assignees_roles')) ? 'checked' : '' }} /> <label for="role-{{ $role->id }}">{{ $role->name }}</label>
                               @else
                                <input type="radio" value="{{ $role->id }}" name="assignees_roles" id="role-{{ $role->id }}" {{ is_array(old('assignees_roles')) && in_array($role->id, old('assignees_roles')) ? 'checked' : '' }} /> <label for="role-{{ $role->id }}">{{ $role->name }}</label>
                                @endif
