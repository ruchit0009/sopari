...
use App\Models\Store\StoreItemModel;
...
class UserRepository extends BaseRepository {
    ...
    public function deActive($id) {
        $store = StoreUserModel::where('user_id', $id)->first();
        StoreModel::where('id', $store['store_id'])->update(['status' => '0']);
        $items = StoreItemModel::where('store_id', $store['store_id'])->get();
        if (!empty($items[0])) {
            foreach ($items as $item) {
                ItemModel::where('id', $item['item_id'])->update(['status' => '0']);
            }
        }
    }
    ...
    public function deleteAll($id) {
        $store = StoreUserModel::where('user_id', $id)->first();
        StoreModel::where('id', $store['store_id'])->update(['deleted_by' => Auth::user()->id]);
        StoreModel::where('id', $store['store_id'])->delete();
        $items = StoreItemModel::where('store_id', $store['store_id'])->get();
        if (!empty($items[0])) {
            foreach ($items as $item) {
                ItemModel::where('id', $item['item_id'])->update(['deleted_by' => Auth::user()->id]);
                ItemModel::where('id', $item['item_id'])->delete();
            }
        }
    }
